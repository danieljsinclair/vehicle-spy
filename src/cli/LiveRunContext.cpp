...
int LiveRunContext::run(
    ...
    // When --stdout-csv is active, route terminal progress to stderr so stdout
    // carries clean CSV rows only.
    std::ostream& displayStream = stdoutCsv ? static_cast<std::ostream&>(std::cerr) : std::cout;
    pipeline::ConsoleProgressReporter progress(displayStream, translationService.getVehicleId());

    // Optional stdout CSV sink for piping to engine-sim-cli.
    std::unique_ptr<telemetry::CsvStdoutSink> stdoutSink;
    telemetry::CsvStdoutSink* stdoutSinkPtr = nullptr;
    if (stdoutCsv) {
        stdoutSink = std::make_unique<telemetry::CsvStdoutSink>(std::cout);
        stdoutSinkPtr = stdoutSink.get();
    }
    ...
    auto stats = pipeline::runReplay(*source.transport, *source.normaliser,
                                     ...
                                     rawSink.get(), &progress, stdoutSinkPtr);
