...
echo "=== Coverage lcov generated (coverage-sonar.xml is a separate make target) ==="
...
# NOTE: coverage-sonar.xml is now generated by the Makefile (not this script),
# so the XML-vs-lcov comparison happens there instead.
...
echo "  (coverage-sonar.xml guard moved to Makefile rule)"
