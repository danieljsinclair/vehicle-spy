...
3. **Flash the firmware** (first boot lands in AP mode):
   ```bash
   make flash
   ```
   The ESP32 boots as `ESP32-CAN` / `cancan12` until credentials are provisioned.

4. **Provision WiFi credentials** (stored in NVS, survives re-flash):
   ```bash
   make set-wifi-creds ESP32_WIFI_SSID="YourNetworkName" ESP32_WIFI_PASS="YourPassword"
   ```
   To reset to factory defaults first:
   ```bash
   make clear-wifi-creds
   ```
   For the road test, the phone hotspot the Mac is also on is usually the right SSID.

5. **Generate OTA credentials** (first time only):
   ```bash
   make ota-creds
   ```
...
| `make set-wifi-creds` | Provision WiFi credentials to NVS (USB preferred, network fallback) |
| `make clear-wifi-creds` | Clear WiFi credentials from NVS — factory reset to default AP (USB preferred, network fallback) |
| `make join-wifi` | Alias for set-wifi-creds |
| `make join-wifi-usb` | Provision WiFi credentials over USB serial only |
| `make clear-wifi-creds-usb` | Clear WiFi credentials over USB serial only |
...
|----------|----------|-------------|
| `ESP32_WIFI_SSID` | For `make set-wifi-creds` | WiFi network name (provisioned to NVS, not baked into firmware) |
| `ESP32_WIFI_PASS` | For `make set-wifi-creds` | WiFi password (provisioned to NVS, not baked into firmware) |
| `ESP32_TCP_TOKEN` | No (default: `vehicle-sim-2026`) | TCP auth token — required for all TCP connections (capture, reboot, OTA, clear-wifi-creds network fallback). Generated by `make ota-creds` and persisted in `~/.zshrc`. The firmware default is baked in but overridable at runtime via `ATSETTOKEN` (stored in NVS); the host binary is baked at build time, so changing the token still requires `make native`/`make ios` to re-bake the host. |
...
# Re-flash over USB: make flash, then make set-wifi-creds ESP32_WIFI_SSID=<ssid> ESP32_WIFI_PASS=<password>
# To reset to factory defaults first: make clear-wifi-creds
```
